 ...
 * @subpackage	Render
 */
class SZend_Calendar_Render_ZendView
    extends SZend_Calendar_Render_Abstract
    implements SZend_Calendar_Render_Interface
{
    ...
    public function render() {
        ...
        foreach ($this->getCalendar()->getWeeks() as $k => $week) {
            foreach ($week as $day) {
                $weekList[$k][] = array(
                    'day' => $day,
                    'events' => $this->getCalendar()->getDayEvents($day,
                                                    SZend_Calendar_Event::ALL));
            }
        }

        $view->calendar->weekList = $weekList;
        $calHeader = $this->getCalendar()->getCalendarHeaderDataArray('user', 'calendar');
        $view->calendar->calHeader = $calHeader;
