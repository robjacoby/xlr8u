 ...
 */
class SZend_Calendar extends My_Service_Calendar {
    const SUNDAY = 0;
    const MONDAY = 1;
    const TUESDAY = 2;
    const WEDNESDAY = 3;
...
//        protected $_focusDate;
     ...
     * @param int $year
     */
    public function getDay($day, $month = null, $year = null, $class = null) {
        if (null === $month) {
        ...
        if (!isset($this->_days[$year][$month][$day])) {
            $this->setDay(mktime(0, 0, 0, $month, $day, $year), null, null, $class);
        } else {
            var_dump($month);
        }
     ...
     * @param string|Zend_Locale $locale
     */
    public function setDay($date, $part = null, $locale = null, $class = null) {
        $day = new SZend_Calendar_Day($date, $part, $locale, $class);
    ...
    protected function _generateWeeks() {
        ...
        $dayArray = $this->getCalendarMonthDayDataArray();
        ...
        for ($i = $firstDay - 1; $i >= 0; $i--) {
            $this->_weeks[1][$i] = $this->getDay($endOfDays--, $prevMonth, null, 'nonmonth');
        }
        if (isset($this->_weeks[1]))
            $this->_weeks[1] = array_reverse($this->_weeks[1]);
        // Current months weeks
        $j = $firstDay;
        foreach ($dayArray as $k => $week){
            foreach ($week as $day){
                if (isset($day['num'])){
                    $this->_weeks[$k+1][$j] = $this->getDay($day['num'], null, null, $day['class']);
                    $j++;
                    if ($j == 7){
                        $j = 0;
                    }
        ...
        for ($i = $j; $i < 7; $i++) {
            if ($j != 0)
                $this->_weeks[$lastIndex][$i] = $this->getDay(($i - $j) + 1, $nextMonth, null, 'nonmonth');
        }
    }
